...
//             if (label) { label += ': '; }
//             if (context.parsed !== null) { label += context.parsed + ' (' + context.dataset.data[context.dataIndex] + '%)'; }
//             return label;
//           }
//         }
//       }
...
//   public islamicSubjectPagesChartType: 'bar' = 'bar';
...
//   public attendanceChartOptions: ChartOptions<'bar'> = {
//     responsive: true,
//     plugins: {
//       legend: { display: true },
...
//     },
//     scales: {
//       y: { title: { display: true, text: 'عدد الحضور' } },
...
//       showMonthlyAttendance: [true],
//       showWeeklyAttendance: [false]
//     });
...
//   const present = this.chartData.find(item => item.status === 'حضور');
...
//   }
// }
// groupAttendanceByHalaqa(): any[] {
//   const halaqaMap = new Map<number, any>();

//   this.chartData.forEach(item => {
//     if (!halaqaMap.has(item.halaqaId)) {
//       halaqaMap.set(item.halaqaId, {
...
//     const halaqa = halaqaMap.get(item.halaqaId);
//     if (item.status === 'حضور') {
//       halaqa.presentCount = item.count;
//     } else if (item.status === 'غياب') {
//       halaqa.absentCount = item.count;
//     } else if (item.status === 'معذور') {
//       halaqa.excusedCount = item.count;
//     }

//     halaqa.total = halaqa.presentCount + halaqa.absentCount + halaqa.excusedCount;
//     halaqa.percentage = (halaqa.presentCount / halaqa.total) * 100;
//   });

//   return Array.from(halaqaMap.values());
// }
...
//     this.attendanceChartData.datasets[0].hidden = !formValues.showMonthlyAttendance;
//     this.attendanceChartData.datasets[1].hidden = !formValues.showWeeklyAttendance;
...
//     // this.loadReportData(studentId);
//     this.loadStudentAttendanceDetails(studentId);
//     this.loadChartData(studentId);
//     this.loadQuranProgressData(studentId);
//     this.loadAttendanceData(studentId);
...
//         console.log('Received student attendance details:', this.studentAttendanceDetails);
...
//     forkJoin([
//       this.reportsService.getStudentMonthlyWeeklyAttendanceChart(studentId, 'month'),
//       this.reportsService.getStudentMonthlyWeeklyAttendanceChart(studentId, 'week')
//     ]).subscribe({
//       next: ([monthlyData, weeklyData]) => {
...
//               data: weeklyData.map(item => item.presentCount)
//             }
//           ]
//         };
//         this.cdr.detectChanges();
...
//     next: (data) => {
//       console.log('Received Islamic subject pages chart data:', data);
//       if (data && data.length > 0) {
...
//         };
//       } else {
//         console.log('Islamic subject pages chart data received is empty or null.');
...
//     },
//     error: (err) => {
//       console.error('Error fetching Islamic subject pages chart data:', err);
//     }
//   });
// }

// // يمكنك إضافة هذه الدالة المساعدة للألوان
